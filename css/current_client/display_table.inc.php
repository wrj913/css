...
<?php
	...
	$query=mysql_query("select * from current_clients");
...
<?php
		echo "<table class='css_table_tr'>
		<tr><td></td><td colspan='9'  style='font-size: 12px; text-align: right;'><a href='./css/current_client/creat_excel.php'>导出</a></td></tr>
		<tr  bgcolor='#00438C'><td colspan='10'  style='font-size: 12px; font-weight: bold;' class='textHeaderDark'>当前终端</td></tr>
...
//		虚拟IP 	MAC地址 	类型 	使用频率 	CSSD 	IPQAM 	来源IP 	状态 	最后接入时间
      ...
      for($j = 2; $j < 11; $j=$j+3){
        $dec_mac=str_insert($dec_mac,$j,'-');
		  ...
		  echo  "<td class="."css_table"."$mantissa>" . $row['updated_at'] . "</td>";
